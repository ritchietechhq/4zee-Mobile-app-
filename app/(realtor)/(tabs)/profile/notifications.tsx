...
const getNotificationIcon = (type: string): { icon: NotificationIcon; color: string; bgColor: string } => {
  const icons: Record<string, { icon: NotificationIcon; color: string; bgColor: string }> = {
    ...
    COMMISSION_APPROVED: { icon: 'cash', color: '#16A34A', bgColor: '#DCFCE7' },
    COMMISSION_PAID: { icon: 'cash', color: '#6366F1', bgColor: '#EEF2FF' },
    PAYOUT_COMPLETED: { icon: 'wallet', color: '#16A34A', bgColor: '#DCFCE7' },
    PAYOUT_FAILED: { icon: 'wallet', color: '#DC2626', bgColor: '#FEE2E2' },
    NEW_PROPERTY: { icon: 'home', color: '#6366F1', bgColor: '#EEF2FF' },
    PROPERTY_UPDATE: { icon: 'home', color: '#6366F1', bgColor: '#EEF2FF' },
    KYC_APPROVED: { icon: 'shield-checkmark', color: '#16A34A', bgColor: '#DCFCE7' },
    KYC_REJECTED: { icon: 'shield', color: '#DC2626', bgColor: '#FEE2E2' },
...
export default function NotificationsScreen() {
  ...
  const handleNotificationPress = (notification: Notification) => {
    ...
    // Navigate based on notification type — realtor-specific routes
    if (notification.data) {
      const data = notification.data as Record<string, string>;
      if (data.commissionId || data.payoutId) {
        router.push('/(realtor)/payments' as any);
      } else if (data.propertyId) {
        router.push(`/(realtor)/listings/${data.propertyId}` as any);
      }
    }
  };
        ...
        <EmptyState
          ...
          description={
            showUnreadOnly
              ...
              : "You don't have any notifications yet. We'll notify you about commissions, payouts, and leads."
          }
