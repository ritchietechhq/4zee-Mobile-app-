...
export default function RequestsScreen() {
  ...
  const fetchApplications = useCallback(async () => {
    try {
      ...
      if (__DEV__) console.log('[Requests/Apps] items:', response.items?.length);
  ...
  const fetchInstRequests = useCallback(async () => {
    try {
      ...
      if (__DEV__) console.log('[Requests/Inst] items:', response.items?.length);
      ...
      <View style={styles.requestCard}>
        ...
        {item.paymentStatus && (
          <View style={styles.paymentStatusRow}>
            ...
            />
            <Text style={[styles.paymentStatusText, { color: item.paymentStatus === 'PAID' ? Colors.success : Colors.warning }]}>
        ...
        {item.status === 'PENDING' && (
          <View style={styles.actions}>
            <Button title="Reject" variant="outline" size="sm" style={styles.actionBtn} onPress={() => openAppRejectModal(item)} disabled={appsProcessing} />
            <Button title="Approve" variant="primary" size="sm" style={[styles.actionBtn, styles.approveBtn]} onPress={() => handleApproveApp(item)} disabled={appsProcessing} icon={<Ionicons name="checkmark" size={16} color={Colors.white} />} />
          </View>
        )}
      </View>
    );
    ...
    return (
      <View style={styles.requestCard}>
        ...
        <View style={styles.clientRow}>
          <View style={[styles.clientAvatar, { backgroundColor: colors.primaryLight }]}>
            <Text style={[styles.clientAvatarText, { color: colors.primary }]}>
              {item.client.firstName?.charAt(0)?.toUpperCase() ?? '?'}
            </Text>
          </View>
          <View style={styles.clientInfo}>
        ...
        <Text style={styles.requestedAt}>
          Requested: {new Date(item.requestedAt).toLocaleDateString('en-NG', { day: 'numeric', month: 'short', year: 'numeric' })}
        </Text>

        {item.status === 'PENDING' && (
          <View style={styles.actions}>
            <Button title="Reject" variant="outline" size="sm" style={styles.actionBtn} onPress={() => openInstRejectModal(item)} disabled={instProcessing} />
            <Button title="Approve" variant="primary" size="sm" style={[styles.actionBtn, styles.approveBtn]} onPress={() => handleApproveInst(item)} disabled={instProcessing} icon={<Ionicons name="checkmark" size={16} color={Colors.white} />} />
          </View>
        )}

        {item.status === 'APPROVED' && item.agreement && (
          <View style={styles.actions}>
            <Button title="View Agreement" variant="primary" size="sm" style={styles.actionBtn} onPress={() => handleViewAgreement(item)} icon={<Ionicons name="document-text-outline" size={16} color={Colors.white} />} />
          </View>
        )}
        ...
        <FlatList
          ...
          ListEmptyComponent={
            <EmptyState
              ...
              description={fetchError ? `Error: ${fetchError}\n\nPull down to retry.` : appFilter === 'ALL' ? "You don't have any property applications yet. They'll appear here when clients apply for your properties." : `No ${appFilter.toLowerCase()} applications found.`}
            />
          }
        ...
        <FlatList
          ...
          ListEmptyComponent={
            <EmptyState
              ...
              description={fetchError ? `Error: ${fetchError}\n\nPull down to retry.` : instFilter === 'ALL' ? "No installment plan requests yet. They'll appear when clients request payment plans for your properties." : `No ${instFilter.toLowerCase()} requests found.`}
            />
          }
          ListFooterComponent={<View style={{ height: Spacing.xxxxl }} />}
          {...FLATLIST_PERF_PROPS}
        ...
        <View style={styles.modalOverlay}>
          <View style={styles.modalContent}>
            <View style={styles.modalHeader}>
              <Text style={styles.modalTitle}>Reject {rejectTarget?.type === 'app' ? 'Application' : 'Request'}</Text>
            ...
            <View style={styles.modalActions}>
              <Button title="Cancel" variant="outline" size="md" style={{ flex: 1, marginRight: Spacing.sm }} onPress={() => setRejectModalVisible(false)} disabled={isProcessing} />
              <Button title={isProcessing ? 'Rejecting...' : 'Reject'} variant="danger" size="md" style={{ flex: 1 }} onPress={handleReject} disabled={isProcessing || !rejectionReason.trim()} />
            </View>
          </View>
        </View>
      </Modal>
...
const makeStyles = (colors: ThemeColors) => StyleSheet.create({
  ...
  segmentWrap: { flexDirection: 'row', marginHorizontal: Spacing.xl, marginVertical: Spacing.sm, backgroundColor: colors.surface, borderRadius: BorderRadius.lg, padding: 3, borderWidth: 1, borderColor: colors.borderLight },
  segmentBtn: { flex: 1, flexDirection: 'row', alignItems: 'center', justifyContent: 'center', paddingVertical: Spacing.sm + 2, borderRadius: BorderRadius.md, gap: 6 },
  segmentBtnActive: { backgroundColor: colors.primary, ...Shadows.sm },
  segmentText: { ...Typography.captionMedium, color: colors.textSecondary },
  segmentTextActive: { color: colors.white },
  segmentBadge: { minWidth: 18, height: 18, borderRadius: 9, backgroundColor: colors.error, alignItems: 'center', justifyContent: 'center', paddingHorizontal: 4 },
