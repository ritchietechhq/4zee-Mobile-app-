...
export default function SuperAdminRealtorsScreen() {
  const colors = useThemeColors();
  const styles = useMemo(() => makeStyles(colors), [colors]);

  const [realtors, setRealtors] = useState<SuperAdminUser[]>([]);
  ...
  const fetchRealtors = useCallback(async (pageNum = 1, append = false) => {
    try {
      const params: Record<string, unknown> = { page: pageNum, limit: 20 };
      if (search.trim()) params.search = search.trim();
      if (kycFilter !== 'ALL') params.kycStatus = kycFilter;
      const res = await adminService.getRealtorUsers(params as any);
      const newData = res.items ?? [];
      setRealtors(append ? (prev) => [...prev, ...newData] : newData);
      setHasMore(newData.length >= 20);
    } catch (e) {
      console.error('Fetch realtors error:', e);
    }
  }, [search, kycFilter]);
  ...
  useEffect(() => {
    setPage(1);
    setIsLoading(true);
    fetchRealtors(1).finally(() => setIsLoading(false));
  }, [kycFilter]);
  ...
  const loadMore = useCallback(async () => {
    ...
    const next = page + 1;
    setPage(next);
    await fetchRealtors(next, true);
    setIsLoadingMore(false);
  }, [hasMore, isLoadingMore, page, fetchRealtors]);

  const formatDate = (dateStr: string) =>
    new Date(dateStr).toLocaleDateString('en-NG', { day: 'numeric', month: 'short', year: 'numeric' });

  const renderRealtor = ({ item }: { item: SuperAdminUser }) => (
    <TouchableOpacity
      style={styles.card}
    ...
    <>
      ...
      <SafeAreaView style={styles.safe}>
        <View style={styles.header}>
          ...
          <View style={{ flex: 1 }}>
            <Text style={styles.headerTitle}>All Realtors</Text>
            <Text style={styles.headerSubtitle}>Admin • Realtor management</Text>
          </View>
        </View>
        ...
        {isLoading ? (
          ...
          <EmptyState icon="briefcase-outline" title="No realtors found" description="Try adjusting your search or filters" />
        ) : (
          <FlatList
            data={realtors}
...
const makeStyles = (colors: ThemeColors) =>
  StyleSheet.create({
    ...
    initials: { ...Typography.bodyMedium, color: colors.teal, fontWeight: '700' },
